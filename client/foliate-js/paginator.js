...
export class Paginator extends HTMLElement {
    ...
    #replaceBackground(background, columnCount) {
        ...
        // 背景は CSS 変数 --theme-bg で制御（子要素には設定しない）
        this.#background.style.background = 'var(--theme-bg)'
    ...
    setStyles(styles) {
        ...
        // 同期的に背景を更新（requestAnimationFrame 不要）
        this.#replaceBackground(this.#view.docBackground, this.columnCount)
    ...
    // 同期的に背景色を更新（DevTools非依存で即座に反映）
    updateBackgroundSync(backgroundColor) {
        if (!this.#background) return;
        this.#background.querySelectorAll('div').forEach(d => {
