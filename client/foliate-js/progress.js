...
export class SectionProgress {
    ...
    #getSectionFractions() {
        ...
        if (!sizeTotal) {
            // sizeTotal が 0 の場合（大きな本で sections が空）は均等分割
            return this.sizes.map((_, i) => i / this.sizes.length)
        }
        const results = [0]
        let sum = 0
    ...
    getSection(fraction) {
        ...
        if (fraction >= 1) return [Math.max(0, this.sizes.length - 1), 1]
        // sizeTotal が 0 の場合（大きな本で sections が空）はセクション数で均等分割
        if (!this.sizeTotal) {
            const idx = Math.min(Math.floor(fraction * this.sizes.length), this.sizes.length - 1);
            return [idx, (fraction * this.sizes.length) - idx];
        }
